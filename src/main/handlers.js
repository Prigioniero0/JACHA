...
module.exports = function registerHandlers() {
    ...
    ipcMain.handle('ssh:generate', (event, { port, osType }) => {
        const portNum = port || 22;
        const type = osType || 'windows';

        if (type === 'linux') {
            const script = `#!/bin/bash
# JACHA Console - Linux SSH Server Setup
# Run as Root (sudo)
...
sed -i "s/#Port 22/Port ${portNum}/g" /etc/ssh/sshd_config
sed -i "s/Port 22/Port ${portNum}/g" /etc/ssh/sshd_config
...
echo "Setup Complete! Listening on Port ${portNum}"
`;
            return { content: script, filename: 'setup_ssh_linux.sh' };
        }
...
`;
            return { content: scriptContent, filename: 'setup_ssh_windows.ps1' };
        } catch (e) {
            return { error: e.message };
        }
    });
    ...
    ipcMain.handle('winrm:generate', (event, { port, osType }) => {
        const portNum = port || 5986;
        const type = osType || 'windows';

        if (type === 'linux') {
            const script = `#!/bin/bash
# JACHA Console - Linux WinRM (OMI) Setup
# This script installs OMI (Open Management Infrastructure) to allow WinRM connections.
# NOTE: This assumes a Debian/Ubuntu based system. Check OMI docs for others.
...
echo "Installing OMI (WinRM for Linux)..."
# Download OMI (Example for x64)
wget https://github.com/microsoft/omi/releases/download/v1.6.8-1/omi-1.6.8-1.ssl_110.ulinux.x64.deb -O omi.deb

echo "Installing Package..."
dpkg -i omi.deb
apt-get install -f -y

echo "Starting OMI Service..."
...
echo "OMI Installed. You may need to configure HTTPS listeners manually in /etc/opt/omi/conf/omiserver.conf"
`;
            return { content: script, filename: 'setup_winrm_linux.sh' };
        }
...
Read-Host "Press Enter to exit"
`;
            return { content: scriptContent, filename: 'setup_winrm_windows.ps1' };
        } catch (e) {
            return { error: e.message };
        }
    });
    ...
    // --- PACKET CRAFTER (NPING) ---
    ipcMain.on('packet:send', (event, options) => {
        const { target, port, protocol, count, payload, flags } = options;
        const { spawn } = require('child_process'); // Ensure spawn is available if not global

        let args = ['--dest-ip', target, '-c', count || '4'];
        ...
        // Payload
        if (payload) {
            args.push('--data-string', payload);
        }

        // Run Nping
        // Note: Nping assumes 'nping' is in PATH (from Nmap install)
        const proc = spawn('nping', args, { shell: true });

        proc.stdout.on('data', (data) => event.reply('packet:data', data.toString()));
        proc.stderr.on('data', (data) => event.reply('packet:data', data.toString()));
        proc.on('close', (code) => event.reply('packet:exit', code));
    });
